...
`mlang-config`; the language server itself is the self-hosted `mlangd-mla`.
The bootstrap manifest in `bootstrap/` uses `mlang pkg` to orchestrate the
self-hosted build order.
...
| `mlang-format`        | `tools/mlang-format-mla/main.mla`   | bootstrap          | Standalone source formatter that reads `.mlang-format` and writes to stdout or in place.            | Command-line and automated code-style enforcement.                                                                                      |
| `mlangd-mla`          | `tools/mlangd-mla/main.mla` + `format_config.mla` | bootstrap | MLang language server with diagnostics, navigation, completion, and config-aware formatting.      | Primary editor integration for VS Code, UVim, Helix, and other LSP clients.                                                             |
...
Build `mlangd-mla` explicitly with the CMake target shown below, or use the
bootstrap manifest's `build-all` task to build the three self-hosted tooling
components and launcher. `build-all` does not build the separate `mlangpkg`
prototype.
...
- `cmake --install build --prefix "$HOME/.local"` installs the CMake-managed
  tools and native pages for `mlang`, `mlang-pkg`, and `mlang-format`
...
`man mlang-pkg`, `man mlang-format`, `man mlangd-mla`, and
...
## LSP
The supported language server is the self-hosted `build/mlangd-mla` binary.

```sh
cmake --build build --target mlangd-mla
build/mlangd-mla --stdio
```

Use it with UVim:

```sh
uvim --mlang-lsp --mlang-lsp-path /path/to/mlangd-mla
```

UVim formatting works through `textDocument/formatting`; `mlangd-mla`
applies `.mlang-format` internally without launching another process.

## Mlangd (Mlang LSP)
The Mlang implementation lives at `tools/mlangd-mla/main.mla`.
It uses `std::jsonrpc::run_stdio_loop(...)` and a Mlang dispatcher hook:
`__mlang_std_jsonrpc_runtime_dispatch(...)`.

Formatting and `.mlang-format` discovery are implemented separately in
`tools/mlangd-mla/format_config.mla`. For whole-document formatting, the
server searches from the document directory toward the workspace root and
applies the nearest `.mlang-format`. Range formatting uses the same spacing
and indentation settings without applying the whole-file trailing-newline
setting.

The built-in LSP formatter accepts the same `.mlang-format` keys documented
in [Formatter](#formatter). It does not need to launch the standalone
`mlang-format` process. When no config file is found, it uses the LSP client's
`tabSize` and `insertSpaces` options together with the default spacing style.
...
[Build ergonomics](docs/package_manager.md#build-ergonomics-profiles-and-features)
