...
comments are allowed between the hint and `if`. For an `else if`, place the
hint after `else`; a plain `else` cannot be annotated because it has no
condition. Using either keyword anywhere other than immediately before a
runtime `if` produces `MLANG-E1018`.
...
alternative: it returns `option<T>`, supports `is_some()` and `is_none()`, and
its `unwrap()` method reports the source location and aborts when the option is
empty. A `let` binding remains immutable even when its type is `mutmultiarray`.
...
[Package-manager ecosystem](docs/package_manager.md#registry-and-supply-chain)
