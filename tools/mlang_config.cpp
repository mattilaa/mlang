...
std::string default_install_prefix()
{
    ...
    if(home.empty())
        return ".local";
    return (fs::path(home) / ".local").string();
}

std::string default_bin_dir()
{
    return (fs::path(default_install_prefix()) / "bin").string();
}
...
struct Config
{
    ...
    std::string installPrefix = default_install_prefix();
    std::string binDir = default_bin_dir();
    std::string jobs;
...
{
    cfg.installPrefix = expand_user_path(cfg.installPrefix);
    cfg.binDir = expand_user_path(cfg.binDir);
}

void save_config(const Config& cfg)
{
    Config normalized = cfg;
    normalize_config_paths(normalized);
    fs::create_directories(cfg.buildDir);
    {
        ...
        out << "build_type=" << normalized.buildType << "\n";
