...
for command in ls cat chmod chown mkdir rm echo cp mv wc vi; do
...
timestamp_index=0
while [ "$timestamp_index" -lt 20 ]; do
...
command_index=2
for command in vi ls cat chmod chown mkdir rm echo cp mv wc; do
...
echo "  native commands: /bin/ls /bin/cat /bin/chmod /bin/chown /bin/mkdir /bin/rm /bin/echo /bin/cp /bin/mv /bin/wc /bin/vi"
