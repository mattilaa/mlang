...
for command in ls cat chmod chown mkdir rm vi; do
...
timestamp_index=0
while [ "$timestamp_index" -lt 16 ]; do
...
command_index=2
for command in vi ls cat chmod chown mkdir rm; do
...
echo "  native commands: /bin/ls /bin/cat /bin/chmod /bin/chown /bin/mkdir /bin/rm /bin/vi"
