...
Usage: mlacker_editor_smoke.py <mlacker> <MlaFilter.vst3>
"""
import os
import sys
import tempfile
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix="mla-filter-ui-") as directory:
        session = Path(directory) / "filter.mlack"
        tui = Terminal()
        try:
            tui.read(.8)
            ...
            expect(tui.send(b"\r"), b"VST3 editor:", b"Type", b"Cutoff", b"Resonance", b"Mix")
            # mlacker represents continuous VST3 controls in normalized 0..1.
            expect(tui.send(b"j\r\x150.25\r"), b"0.25")
            # Scroll through all seven controls.
            expect(tui.send(b"j" * 5), b"Bypass")
            tui.send(b"\x1b")
            tui.send(b"\x13")
        ...
        try:
            ...
            expect(tui.send(F1 + b"llllllljj\r"), b"VST3 editor:", b"Cutoff", b"0.25")
        finally:
            tui.close()
    print("Mla Filter mlacker editor smoke passed")
