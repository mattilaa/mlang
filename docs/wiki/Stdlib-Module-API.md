...
backing type such as [`u8`](Quick-Guide#types) or [`i64`](Quick-Guide#types), or [`str8`](Quick-Guide#types) for string-backed enums.
...
- [`str8`](Quick-Guide#types)-backed enums compare directly with [`str8`](Quick-Guide#types).
- `for value in Status { ... }` iterates the enum in declaration order.
- `for (i, value) in Status.enumerate() { ... }` iterates in declaration order
  and exposes a zero-based [`i64`](Quick-Guide#types) index.
...
### [`list<T>`](Quick-Guide#types) / [`Vec<T>`](Stdlib-Module-API#stdvec)

[`Vec<T>`](Stdlib-Module-API#stdvec) is a type alias for [`list<T>`](Quick-Guide#types), so both expose the same method surface.
...
[`alias`](Language-Syntax) lets you introduce readable names for builtin or generic types and the alias respects lexical scope.
...
The older `use type SomeMap<K, V> = map<K, V>;` spelling remains valid and equivalent; prefer [`alias`](Language-Syntax) for ordinary type alias declarations.
...
Ordered-sequence helpers for [`list<i64>`](Quick-Guide#types).
...
- `unique_i64(data: &list<i64>) -> list<i64>` (adjacent duplicate removal, [`std::ranges::unique`](Stdlib-Module-API#stdranges)-style)
...
- Use [`std::bitset::BitSet`](Stdlib-Module-API#stdbitset) when you need one-bit-per-entry dense storage
...
Sequences auto-disable to `""` when [`std::term::supports_ansi()`](Stdlib-Module-API#stdterm) is false.

Ownership note:
- most [`std::esc`](Stdlib-Module-API#stdesc) helpers return borrowed/static escape strings
...
### Wire format
- magic: `MLP1` (4 bytes)
- opcode: big-endian [`u32`](Quick-Guide#types)
- payload length: big-endian [`u32`](Quick-Guide#types)
- payload bytes
...
- caller frees returned strings with [`std::strbuf::free`](Stdlib-Module-API#stdstrbuf)
...
The [`BitSet`](Stdlib-Module-API#stdbits) overloads operate on packed one-bit-per-entry storage from
[`std::bitset::BitSet`](Stdlib-Module-API#stdbitset). The `_eq` functions mutate the left-hand bitset in
...
- [`Span<T>`](Stdlib-Module-API#stdspan) is a compiler alias for [`list<T>`](Quick-Guide#types)
...
[`Vec<T>`](Stdlib-Module-API#stdvec) is a type alias for [`list<T>`](Quick-Guide#types). The two are interchangeable in all
...
- `Vec::new_i32() -> Vec<i32>` — empty Vec with explicit [`i32`](Quick-Guide#types) element type
- `Vec::new_i64() -> Vec<i64>` — empty Vec with explicit [`i64`](Quick-Guide#types) element type
- `Vec::new_str() -> Vec<str8>` — empty Vec with explicit [`str8`](Quick-Guide#types) element type
