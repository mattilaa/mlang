...
- result type must be integer, pointer, or [`void`](Quick-Guide#types)
- non-[`void`](Quick-Guide#types) asm uses the first operand as the tied input/output operand
...
text editor built on [`std::esc::freestanding`](Stdlib-Esc).
...
FFT API ([`dsp::fft`](DSP-FFT) forward/inverse on split real/imag arrays) example:
...
analyzer with colorized stereo differences (left/right/overlap) using [`dsp::fft`](DSP-FFT).
...
| Feature | Commit | Demo |
|---|---|---|
| Method visibility on [`impl`](Language-Syntax) blocks | `bd4b339` | `examples/method_visibility_demo/` |
| Associated (static) function calls | `3519d6b` | `examples/associated_functions_demo/` |
| Trait [`impl`](Language-Syntax) method signature validation | `643a182` | `tests/std_compiler_tests.mla` |
...
### Method Visibility On [`impl`](Language-Syntax) Blocks

> **@brief** Allow `pub` and private methods inside [`impl`](Language-Syntax) blocks, with
...
> module where the [`impl`](Language-Syntax) block lives; methods marked `pub fn` are callable
...
> instance methods (see "Method Visibility On [`impl`](Language-Syntax) Blocks").
...
### Trait [`impl`](Language-Syntax) Method Signature Validation
...
> the offending [`impl`](Language-Syntax) block quickly.
...
> structurally incorrect [`impl`](Language-Syntax) blocks cannot ride along quietly until first
> use.
...
> on [`struct`](Quick-Guide#types), [`impl`](Language-Syntax), and (transitively) function definitions, so generic
...
> once, using `+` to chain bounds in struct, [`impl`](Language-Syntax), and type-alias headers.
...
|---|---|---|
| `#[derive(Debug)]` | [`struct`](Quick-Guide#types) definitions | Enables debug formatting (`{:?}`/`{:#?}` and `println!(value)` for structs). |
| `#[derive(Json)]` | [`struct`](Quick-Guide#types) definitions | Synthesizes `to_json()` / `from_json(...)` for supported structs, including inherited fields and `@property` metadata. |
| `#[test]` | [`fn`](Language-Syntax) definitions | Marks test functions discoverable by `mlang test` / `mlang run tests`. |
...
fields are [`bool`](Quick-Guide#types), integer primitives, [`f32`](Quick-Guide#types)/[`f64`](Quick-Guide#types), [`str8`](Quick-Guide#types), or nested structs
