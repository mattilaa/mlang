...
Prefix a runtime [`if`](Language-Syntax) with `likely` or `unlikely` to tell the optimizer which
...
Import [`std::matrix`](Stdlib-Matrix) for numeric operations. Element-wise `add`, `subtract`,
...
matrix multiplication, and `transpose` swaps rows and columns. Square [`f32`](Quick-Guide#types)
and [`f64`](Quick-Guide#types) matrices also provide `determinant` and `inverse`; real symmetric
...
square size, for example `--usesquarefloats --size=4x4`, to use [`f64`](Quick-Guide#types) values
...
[`std::audio::AudioInsertStack`](Stdlib-Audio) can route the default stereo input through up to
...
[std::audio documentation](Stdlib-Audio#insert-stacks-and-effect-racks)
and `examples/std_audio_insert_stack_demo.mla` for a live CoreAudio/JACK demo.

For session-style routing, [`std::audio::AudioMixer`](Stdlib-Audio) provides Audio From/Audio To
