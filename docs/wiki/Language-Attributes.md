...
|---|---|---|
| `#[derive(Debug)]` | [`struct`](Quick-Guide#types) definitions | Enables debug formatting for structs. |
| `#[derive(Json)]` | [`struct`](Quick-Guide#types) definitions | Synthesizes JSON serialization/deserialization helpers for supported structs. |
| `#[test]` | [`fn`](Language-Syntax) definitions | Marks a function as a test case. |

## `#[derive(Debug)]`

Applies to [`struct`](Quick-Guide#types) definitions (including `pub struct` and generic structs).
...
Applies to [`struct`](Quick-Guide#types) definitions, including derived structs and generic
structs.
...
- Return type must be [`void`](Quick-Guide#types) or [`i32`](Quick-Guide#types).
...
Applies to inherent [`impl`](Language-Syntax) blocks. Marks the impl as a *test fixture*: every
...
- Must return [`void`](Quick-Guide#types) or [`i32`](Quick-Guide#types).
...
These attributes are intended for cases where ordinary [`if`](Language-Syntax) branches are not
...
Unlike ordinary [`if`](Language-Syntax) statements, non-matching regions are filtered out before
...
- `get<Field>() -> T`
- `set<Field>(value: T) -> void` for mutable [`var`](Language-Syntax) fields
...
Supported options:
- `@property(atomic)`:
  Generates atomic load/store accessors for integer and [`bool`](Quick-Guide#types) fields.
- `@property(mutex)`:
