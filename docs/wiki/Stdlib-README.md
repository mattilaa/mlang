...
- `std/regex.mla`: POSIX regular expression helpers
  ...
  with error retrieval via [`std::regex::last_error()`](Stdlib-Module-API#stdregex).
- `std/algorithm.mla`: algorithm namespace root with submodules:
  - [`std::algorithm::fuzzy`](Stdlib-Module-API#stdalgorithm): subsequence fuzzy matching/ranking
    (`is_match`, `score`, `filter_indices`, `best_index`)
  - [`std::algorithm::order`](Stdlib-Module-API#stdalgorithmorder): ordered-sequence helpers for [`list<i64>`](Quick-Guide#types)
    (`sort_i64`, `reverse_i64`, `is_sorted_i64`, `unique_sorted_i64`,
     `lower_bound_i64`, `upper_bound_i64`, `binary_search_i64`)
  - [`std::algorithm::numeric`](Stdlib-Module-API#stdalgorithmnumeric): numeric helpers for [`list<i64>`](Quick-Guide#types)
    (`accumulate_i64`, `partial_sum_i64`, `adjacent_difference_i64`,
     `inner_product_i64`)
  - [`std::algorithm::ranges`](Stdlib-Module-API#stdalgorithmranges): C++20 ranges-style helpers for [`list<i64>`](Quick-Guide#types)
    (`unique_i64`, `unique_stable_i64`, `find_i64`, `count_i64`,
     `contains_i64`, `remove_i64`, `take_i64`, `drop_i64`)
  - [`std::algorithm::rand`](Stdlib-Module-API#stdalgorithm): random helpers mirroring [`std::rand`](Stdlib-Module-API#stdrand)
  - [`std::algorithm::regex`](Stdlib-Module-API#stdalgorithm): regex helpers mirroring [`std::regex`](Stdlib-Module-API#stdregex)
  - [`std::algorithm::fft`](Stdlib-Module-API#stdalgorithmfft): FFT helpers for forward/inverse transforms
  - [`std::algorithm::genetic`](Stdlib-Module-API#stdalgorithm): GA helpers for genome/population evolution
- `std/esc.mla`: ANSI terminal escape helpers with named color/cursor values
  ...
  `cursor`, `cursor_move`), auto-disabled when [`std::term::supports_ansi()`](Stdlib-Module-API#stdterm)
  is false.
