...
Import [`std::audio::controller`](Stdlib-Audio) for the separate output-only AUHAL API on macOS.
...
JACK2 server first; [`std::audio`](Stdlib-Audio) creates stereo JACK output ports and connects
