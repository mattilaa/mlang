...
### [`list<bool>`](Quick-Guide#types) vs [`std::bitset::BitSet`](Stdlib-std-bitset)
...
- Use [`std::bitset::BitSet`](Stdlib-std-bitset) when you need one-bit-per-entry dense storage
