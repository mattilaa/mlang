...
[`std::matrix`](Stdlib-Matrix) documents numeric operations on fixed-size [`multiarray`](Stdlib-Multiarray) and
[`mutmultiarray`](Stdlib-Mutmultiarray) values. Operands must be fully initialized; a runtime shape
...
immutable [`multiarray`](Stdlib-Multiarray):
