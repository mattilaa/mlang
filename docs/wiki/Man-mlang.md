...
.TP
.B \-\-shared
Build a dynamic/shared library and do not require an executable
.B main
function. The default output extension is
.B .dylib
on macOS,
.B .so
on Linux, and
.B .dll
on Windows; MinGW builds also emit the corresponding import library.
.TP
.B \-\-static\-library
Build a static archive and do not require an executable
.B main
function. The default output is
.IR libSOURCE.a .
.TP
.B \-S
