...
- `set_gate(0.05..1)`: the audible share of each cell; the rest is silent.
- `set_fade_ms(0..50)`: fade-in on each repeat and fade-out before the gate,
  the end of the recording, and on release. `0` gives sample-exact repeats.
- `set_mix(0..1)`: the stutter's share of the output while it is engaged.
  Outside a stutter the input passes unchanged at any mix.

`is_active()` reports a capture or repeat in progress, and `reset()` forgets
both the recording and the grid position while keeping the storage.
