...
`dsp::fft`; the FFT kernel is not itself a SIMD intrinsic API.
Use `dsp::fft` directly when you want the transform module without
...
`std::bitset::bit_set`. The `_eq` functions mutate the left-hand bitset in
