...
| SIMD vector math | Add, subtract, multiply, horizontally sum numeric vectors, and run integer gate/shift operations using `std::simd` over normal `list<i32>`, `list<i64>`, `list<f32>`, and `list<f64>` values; see [`examples/std_simd_demo.mla`](../examples/std_simd_demo.mla). Reference: [Stdlib Module API](stdlib_mlang_api.md#stdsimd). |
