...
| SIMD vector math | Add, subtract, multiply, horizontally reduce numeric vectors, compute running sums, find min/max values, run boolean `any`/`all` checks, and perform integer gate/shift operations using `std::simd` over normal lists; see [`examples/std_simd_demo.mla`](../examples/std_simd_demo.mla). Reference: [Stdlib Module API](stdlib_mlang_api.md#stdsimd). |
