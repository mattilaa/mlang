...
| MIDI input and output | Discover MIDI inputs/outputs, poll owned packets, monitor queue drops, send arbitrary MIDI bytes, and use Note On/Off, control-change, program-change, and pitch-bend helpers through `std::midi`; CoreMIDI is used on macOS and JACK MIDI on Linux. See [`examples/std_midi_demo.mla`](../examples/std_midi_demo.mla). Reference: [Stdlib Module API](stdlib/std_midi.md). |
