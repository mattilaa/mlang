...
| System IPC and named pipes | Exchange bytes between processes with `std::ipc::NamedPipe`, backed by Unix FIFOs or Windows named pipes; see [`examples/std_ipc_named_pipe_demo.mla`](../examples/std_ipc_named_pipe_demo.mla). Reference: [Stdlib Module API](stdlib_mlang_api.md#stdipc). |
