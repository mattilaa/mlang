...
| Fixed-size matrix operations | Use `std::matrix` for N-dimensional element-wise arithmetic and sums, scalar offset/scale, rectangular 2D matrix multiplication, and mutable in-place transforms; see [`examples/matrix_operations.mla`](../examples/matrix_operations.mla). Reference: [`std::matrix`](stdlib/std_matrix.md). |
